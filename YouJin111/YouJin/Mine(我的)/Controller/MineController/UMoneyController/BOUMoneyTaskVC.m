...
- (void)configureViews {
    [self.tableView registerClass:[BOMineTaskCell class] forCellReuseIdentifier:ID];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BOMineTaskCell *cell = [tableView dequeueReusableCellWithIdentifier:ID forIndexPath:indexPath];
    NSArray *dataArrays = self.monthDic[self.monthKeys[indexPath.section]];
    [cell updateItems:dataArrays[indexPath.row]];
    return cell;
}
...
- (void)loadDataWithStartCount:(NSInteger)startCount limit:(NSInteger)limitCount {
    ...
    NSString *url = [NSString stringWithFormat:@"%@Ucenter/getMyQujinApplyList",BASEURL];
    ...
    [self.mgr POST:url parameters:dictionary progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (responseObject[@"data"] && ![responseObject[@"data"] isKindOfClass:[NSNull class]]) {
            NSArray *newDataArray = [UMoneyRecordItem mj_objectArrayWithKeyValuesArray:responseObject[@"data"]];
...
- (void)sortOutDataArray:(NSArray<UMoneyRecordItem *> *)dataArray {
    if (!dataArray || dataArray.count == 0) return;
    for (UMoneyRecordItem *items in dataArray) {
